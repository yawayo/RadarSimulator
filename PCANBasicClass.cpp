...
void PCANBasicClass::LoadAPI()
{
	...
	if(!LoadDllHandle())
    {
        ::MessageBox(NULL,L"Error: \"Unable to find the DLL PCANBasic.dll!\"",L"Error!",MB_ICONERROR);
		return;
	}
    ...
    //
    if(!m_bWasLoaded)
        ::MessageBox(NULL,L"Error: \"DLL functions could not be loaded!\"",L"Error!",MB_ICONERROR);
}
...
{   
	...
	//Loads Dll
	//
	m_hDll = LoadLibrary(L"PCANBasic");
